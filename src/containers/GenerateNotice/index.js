...
export default function GenerateNotice({ tempState, settempState }) {
  ...
  const onSubmit = (e) => {
    ...
    const data = JSON.stringify({
      NoticeID: `NOTICE${Math.floor(new Date().getTime() / 1000)}${Math.floor(
        Math.random(100)
      )}`,
      ...NoticeData,
    });
      ...
      <form autoComplete="off" onSubmit={onSubmit} id="NoticeForm">
        ...
        <Checkbox
          onChange={(e) => {
            setNoticeData({
              ...NoticeData,
              forAnySpecificStreamOrBranch: e.target.checked,
            });
          }}
        >
        ...
        <Checkbox
          onChange={(e) => {
            setNoticeData({
              ...NoticeData,
              ApplicationSpecific: e.target.checked,
            });
          }}
        >
        ...
        <Checkbox
          onChange={(e) => {
            setNoticeData({ ...NoticeData, anyDueAttach: e.target.checked });
          }}
        >
          Any Due Attached ?
        </Checkbox>
        <Checkbox
          onChange={(e) => {
            setNoticeData({ ...NoticeData, anyFileAttached: e.target.checked });
          }}
        >
          File Attached ?
        </Checkbox>
